...
( cd "${dir}" && bjam -d+2 --prefix="${prefix}" --enable-multitarget --with-binutils=2.22.51 --with-gmp-for-gcc=latest --with-mpfr-for-gcc=3.0.1  --with-mpc-for-gcc=latest --with-isl-for-gcc=system --with-cloog-for-gcc=system --enable-compilers=gcc-previous                            --enable-gmp --enable-mpfr --enable-mpc --enable-isl --enable-cloog --enable-ppl --enable-icu4c --with-mpi-backend=openmpi --enable-clang       --enable-valgrind --enable-boost --concurrency=3 --with-awacs=\"${prefix}/bin/twitter.rb\" --with-stream="$log_file_full_path" variant=release link=shared runtime-link=shared threading=multi intro-address-model=64 intro-instruction-set=unspecified std=c++03 lto=off memory-checker=off libstdc++-debug-mode=off >> "$log_file_full_path" 2>&1 )
