...
cd "$dir" && bjam -d+2 --triplet=x86_64-unknown-linux-gnu --prefix="$prefix" --with-binutils-for-gcc=2.22.52 --with-mpfr-for-gcc=3.0.1 --with-ppl-for-gcc=0.11.2 --enable-compilerss=gcc-previous,gcc-previous-snapshot,gcc-current,gcc-current-snapshot,gcc-snapshot --disable-multilib --enable-gmp --enable-mpfr --enable-mpc --enable-ppl --enable-cloog --with-mpi=openmpi --enable-clang --concurrency=3 --with-awacs=\"${prefix}/bin/twitter.rb\" --with-stream="$log_file_full_path" >> "$log_file_full_path" 2>&1
